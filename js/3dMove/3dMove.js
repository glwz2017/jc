;(function ($, window, document, undefined) {
    var Carousel = function (elem, options) {
        ...
        var self = this;
        this.$carousel = elem;
        this.$aImg = this.$carousel.find('.it');
        ...
        this.$aNav = this.$carousel.siblings('#bannerNav').find('ul li'); //切换的子元素
    };

    var outerWidth = parseInt(document.body.offsetWidth);
    var middleWidth = 1920;
    var _height = outerWidth >= middleWidth ? 400 : 360;    //当前激活高度
    var _slideHeight = outerWidth >= middleWidth ? 144 : 144; //未激活高度
    ...
    Carousel.prototype = {
        ...
        initalCarousel: function () {
            ...
            for (var i = 0; i < half_imgLen; i++) {
                ...
                if (k == 0) {
                    this.$aImg.eq(leftNum).css({
                        // transform: 'translateX(' + (-400 * (i + 1)) + 'px) translateZ(-120px)',
                        //左侧位移，-320不同情况不同调整   z轴火狐注意处理
                        transform: 'translateX(' + (-320 * (i + 1)) + 'px) translateZ(0px)',
                        width: "560",
                    }).animate({
                        height: _slideHeight + 'px',
                        marginTop: -_slideHeight / 2 - 10 + 'px',
                        opacity: '0.6',
                    ...
                    this.$aImg.eq(rightNum).css({
                        // transform: 'translateX(' + (400 * (i + 1)) + 'px) translateZ(-120px) ',
                        //右侧位移，400不同情况不同调整   z轴火狐注意处理
                        transform: 'translateX(' + (400 * (i + 1)) + 'px) translateZ(0px) ',
                        width: "560"
                    }).animate({
                        height: _slideHeight + 'px',
                        marginTop: -_slideHeight / 2 - 10 + 'px',
            ...
            //火狐
            this.$carousel.on('transitionend', function () {
                self.b_switch = true;
            });
            //IE
            this.$carousel.on('MSTransitionEnd', function () {
                self.b_switch = true;
            });
        },
        ...
        inital: function () {
            ...
            //IE
            this.$carousel.on('onselectstart', function () {
                return false;
            });
        },
